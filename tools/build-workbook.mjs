...
note(inp,26,'Source: Gorman-Rupp FY2025 earnings release, February 6, 2026.');v(inp,'C27',source.url);inp.getRange('C27').format.font={name:'Arial',size:10,color:green};
note(inp,29,'The release’s condensed statements are unaudited. Data retrieved September 23, 2026.');
...
note(inp,32,'The model uses operating income + D&A, not company-adjusted EBITDA.');
...
for(const r of [15,16,17,18,19,22,23,24,25,26,27,28,32,33,34,35,37,38])a.getRange(`E${r}`).setNumberFormat(percent);
...
const forecastLabels={7:'Organic revenue',8:'Organic revenue growth',9:'Operating EBITDA margin before initiatives',10:'Total revenue',11:'Recurring EBITDA',12:'Implementation expense',13:'EBITDA after implementation',14:'Depreciation and amortization',15:'Operating income',16:'Cash interest expense',17:'Cash taxes',18:'Capital expenditure',19:'Closing operating working capital',20:'Change in operating working capital',21:'Free cash flow before debt service',22:'Unlevered free cash flow',23:'Net income (simplified)',27:'Year-end discount factor',28:'PV of unlevered free cash flow'};
...
const debtLabels={7:'Opening term loan',8:'Opening revolver',9:'Cash interest expense',10:'Opening cash',11:'Free cash flow before debt service',12:'Mandatory term amortization',13:'Cash after minimum cash and amortization',14:'Revolver draw',15:'Unfunded liquidity requirement',16:'Cash available for sweep',17:'Revolver repayment',18:'Term loan cash sweep',19:'Closing term loan',20:'Closing revolver',21:'Closing cash',22:'EBITDA / interest coverage',23:'Year-end net leverage',24:'Debt roll-forward difference',25:'Cash roll-forward difference'};
...
val.getRange('E25').setNumberFormat('0.00;(0.00);0.00');note(val,55,'USD millions. No distributions before exit; annual IRR = MOIC^(1/5) - 1.');note(val,56,'DCF discount rate is assumed. Terminal working capital is recomputed at terminal growth.');
...
const ranges={Summary:['C2:I34'],Assumptions:['C2:I38','C41:I62'],Forecast:['C2:I33'],Debt:['C2:I29'],Valuation:['C2:I37','C40:I56'],Operations:['C2:I24'],'Company Screen':['C2:M24','C28:M46'],Inputs:['C2:I32']};
for(const [name,rs]of Object.entries(ranges))for(const [i,range]of rs.entries()){
  if(process.env.RENDER_CHANGED&&!['Summary','Assumptions','Forecast'].includes(name))continue;
